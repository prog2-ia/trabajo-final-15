...
def cargar_clientes():
    ...
    ruta = os.path.join(BASE_DIR, 'datos', 'dic_clientes.py')

    # si no existe fichero, no hace nada
    if not os.path.exists(ruta):
        return {}

    # importar dinamicamente el fichero
    import importlib.util

    spec = importlib.util.spec_from_file_location("dic_clientes", ruta)
...
def guardar_clientes():
    ...
    with open(ruta, "w", encoding="utf-8") as f:
        ...
        items = list(clientes.items())

        for i, (dni, c) in enumerate(items):
            ...
            f.write(f"    {repr(dni)}: {{\n")
            f.write(f"        'nombre': {repr(c._nombre)},\n")
            f.write(f"        'apellidos': {repr(c._apellidos)},\n")
            f.write(f"        'direccion': {direccion},\n")
            f.write(f"        'pedidos_en_curso': {pedidos_curso},\n")
            f.write(f"        'pedidos_terminados': {pedidos_terminados},\n")
            f.write(f"        'importe_facturado': {c._importe_facturado}\n")
